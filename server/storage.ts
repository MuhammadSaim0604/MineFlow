...
export interface IStorage {
  ...
  createGoogleUser(data: { email: string; username: string; googleId: string; referralCode: string; referredBy?: string }): Promise<User>;
...
export class DatabaseStorage implements IStorage {
  ...
  async createGoogleUser(data: { email: string; username: string; googleId: string; referralCode: string; referredBy?: string }): Promise<User> {
    const tempId = crypto.randomUUID();
    const walletAddress = generateWalletAddress(tempId);
